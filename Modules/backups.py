...
import jarvis.modules.sql.blacklist_sql as blacklistsql

# from jarvis.modules.sql import cust_filters_sql as filtersql
# import jarvis.modules.sql.welcome_sql as welcsql
import jarvis.modules.sql.locks_sql as locksql
import jarvis.modules.sql.notes_sql as sql

# from jarvis.modules.rules import get_rules
import jarvis.modules.sql.rules_sql as rulessql
from jarvis import EVENT_LOGS, LOGGER, OWNER_ID, SUPPORT_CHAT, dispatcher
...
def export_data(update, context):
    ...
    with open("jarvis{}.backup".format(chat_id), "w") as f:
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("jarvis{}.backup".format(chat_id), "rb"),
        caption="📤*Successfully Exported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `jarvis-Backup` was specially made for notes 📚.".format(
            chat.title, chat_id, tgl
        ),
        timeout=360,
    ...
    os.remove("jarvis{}.backup".format(chat_id))  # Cleaning file
